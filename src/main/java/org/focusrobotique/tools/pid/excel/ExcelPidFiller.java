...
public class ExcelPidFiller {
	...
	public void fill(File excelModelFile, PidSampleLineList pidSampleList, File excelTargetFile) throws Exception {
		try (XSSFWorkbook wb = new XSSFWorkbook(excelModelFile)) {
			XSSFSheet sheet = wb.getSheet("Data");
			int rowIndex = 1;
			for (PidSampleLine line : pidSampleList) {
				...
				getOrCreateCell(row, col++).setCellValue(line.getNormalAcceleration());
				getOrCreateCell(row, col++).setCellValue(line.getRealAcceleration());
